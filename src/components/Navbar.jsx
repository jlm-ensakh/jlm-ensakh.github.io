...
const Navbar = () => {
    ...
    const [displayDropMenu, setDisplayDropMenu] = useState(true);
            ...
            <div className="py-4 mx-auto w-full max-w-screen-2xl h-20 flex justify-between items-center">
                ...
                {displayDropMenu && (
                    <div className="absolute block md:hidden top-20 left-0 w-full px-5 pb-5 flex flex-col gap-1 bg-white">
                        {NAV_MENU.map(({ label, target, isButton }) => (
                            <a href={target} className={`py-2 px-4 w-full text-center rounded ${isButton ? "text-white font-semibold bg-orange-400 hover:bg-orange-500" : "hover:bg-gray-100"}`}>{displayDropMenu}{label}</a>
                        ))}
                    </div>
                )}
                ...
                <div className="hidden md:flex gap-2">
                    {NAV_MENU.map(({ label, target, isButton }) => (
                        <a href={target} className={`py-2 px-4 text-center rounded ${isButton ? "text-white font-semibold bg-orange-400 hover:bg-orange-500" : "hover:bg-gray-100"}`}>{label}</a>
                    ))}
                </div>
